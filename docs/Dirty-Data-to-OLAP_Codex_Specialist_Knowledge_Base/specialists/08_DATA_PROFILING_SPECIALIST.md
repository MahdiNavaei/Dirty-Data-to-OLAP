...
# 08 — Data Profiling Specialist Playbook

## 1. Mission in this project

Produce trustworthy statistical and structural observations about tables/columns, with sampling scope and uncertainty made explicit.

This is not a generic job description. It is the operating manual Codex should retrieve when acting as **Data Profiling Specialist** for Dirty Data to OLAP.
...
- semantic/primitive type observations
- sampling-aware interpretation
- profile normalization from DataProfiler
- profiling performance/accuracy tradeoffs

## 5. This specialist explicitly does NOT own

- business severity of quality defects
...
- which metrics are computed for which types
- sample size strategy for expensive statistics
- when exact distinct count is required
- how semantic labels are represented as evidence
- how profile changes across runs are compared
...
- wrap DataProfiler outputs into internal contracts
- calculate/retain null, distinct, length, numeric/date distributions and pattern evidence
- record full vs sample scope everywhere
- detect unstable or mixed physical types
- provide candidate identifier/type evidence without promoting it to truth
- avoid expensive correlation work unless relevant
- build profile diff capability for benchmark/regression

## 9. Expected artifacts / likely repository areas

- src/profiling/
- tests/profiling/
- docs/profiling/metrics.md
- fixtures/profiling/
...
- known-distribution synthetic tests
- sampling reproducibility
- null/distinct exactness tests on small data
- mixed-type/dirty string tests
- semantic label regression tests
- large-column memory tests
...
- calling sample metrics exact
- equating uniqueness with primary key
- running every expensive statistic on every column
- letting profiler labels become canonical semantic types without fusion
...
Primary collaborators: **Data Quality Engineer, Dependency Discovery Engineer, Schema Matching Engineer, Data Engineer, ML Evaluation Engineer**.
...
1. Profile phone columns with mixed formatting.
2. Compare sample vs full-table uniqueness error.
3. Normalize DataProfiler data-label output to Evidence records.
...
When the user or routing layer says **"act as Data Profiling Specialist"**, use this document as the primary role contract. Be proactive inside this role's ownership, but do not override authoritative decisions from the base reports. If implementation and documentation disagree, inspect evidence, report the discrepancy, and prefer correcting the inconsistency over silently choosing one side.
