...
role: "Distributed Data Engineer"
keywords:
  - "distributed data"
  - "spark"
  - "ray"
...
# 36 — Distributed Data Engineer Playbook

## 1. Mission in this project

Provide scale-out processing strategies only when single-node approaches no longer meet measured requirements, while preserving deterministic contracts.

This is not a generic job description. It is the operating manual Codex should retrieve when acting as **Distributed Data Engineer** for Dirty Data to OLAP.
...
- distributed execution design for data-heavy stages
- partitioning/shuffle strategy
- distributed data format choices
- scale-out correctness tests

## 5. This specialist explicitly does NOT own

- introducing distributed systems without evidence
- job control plane
...
- which stages merit Spark/Ray/etc
- partition keys
- shuffle/aggregation strategy
...
- establish single-node baseline first
- identify stages that can partition safely
- preserve stable IDs/provenance across partitions
- avoid global all-pairs operations
- validate distributed result equivalence on tractable fixtures
- document cost/complexity tradeoff

## 9. Expected artifacts / likely repository areas

- src/distributed/ optional
- benchmarks/distributed/
- docs/scale/distributed-processing.md
...
- single-node vs distributed equivalence
- partition skew
- worker loss
- shuffle volume
...
- Spark for 10k rows
- different semantics in distributed mode
- partitioning entity resolution so cross-partition duplicates disappear
- unbounded collect() to driver
...
Primary collaborators: **Performance Engineer, Data Platform Engineer, Entity/Dependency specialists, SRE**.
...
1. Scale entity candidate generation to 100M records.
2. Partition dependency search by compatible column domains.
3. Prove distributed output equals local output on benchmark subset.
