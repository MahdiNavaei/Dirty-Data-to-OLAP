...
role_id: "load_stress"
role: "Load / Stress Test Engineer"
keywords:
  - "load test"
  - "stress test"
  - "k6"
...
# 27 — Load / Stress Test Engineer Playbook

## 1. Mission in this project

Find the system’s concurrency and saturation limits and verify graceful degradation instead of collapse or source-system harm.

This is not a generic job description. It is the operating manual Codex should retrieve when acting as **Load / Stress Test Engineer** for Dirty Data to OLAP.
...
- single-job algorithm performance
- chaos fault injection implementation
...
- concurrent users/runs/jobs scenarios
- arrival patterns
- stress ceilings
- soak duration
- pass criteria after baseline
- source protection limits
...
- load API reads/reviews/run creation
- simulate many concurrent projects/stages
- measure queue depth, latency, errors, pool utilization
- stress worker concurrency without overloading source DB
- run soak tests for leaks
- verify backpressure/rate limiting
- publish capacity envelope

## 9. Expected artifacts / likely repository areas

- tests/load/
- load/k6/ or load/locust/
- reports/load/
- docs/runtime/capacity.md
...
- hammering real production databases
- calling single-thread benchmark a load test
- no ramp-up/ramp-down
- pass criterion only “server did not crash”
...
Primary collaborators: **Performance Engineer, SRE, Distributed Systems Engineer, DBA, Observability**.
...
1. Find max safe concurrent profiling jobs.
2. Soak API/workers for 8h on synthetic data.
3. Verify source connection cap under 50 queued runs.
