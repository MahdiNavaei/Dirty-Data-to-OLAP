...
components:
  - "staging storage"
  - "control store"
  - "compute/runtime infrastructure"
  - "destinations"
status: "authoritative-specialist-guidance"
version: "1.0"
---

# 35 — Data Platform Engineer Playbook

## 1. Mission in this project

Provide scalable, replaceable platform primitives for metadata, staged datasets and compute while keeping V1 locally operable.

This is not a generic job description. It is the operating manual Codex should retrieve when acting as **Data Platform Engineer** for Dirty Data to OLAP.
...
- platform abstraction for storage/compute
- Parquet/object-store strategy beyond local mode
- metadata/control DB operational architecture
- resource isolation
...
- local DuckDB vs external target capability
...
- keep storage interfaces independent of local paths
- define staged dataset naming/versioning
- prepare Postgres control-store option
- support local-first defaults
- document scale-out architecture without prematurely building it
- integrate resource quotas where deployment requires

## 9. Expected artifacts / likely repository areas

- src/platform/
- docs/platform/storage.md
- docs/platform/scale-out.md
- tests/platform/
...
- local filesystem/object-store adapter parity if both exist
- artifact version isolation
- concurrent run storage collisions
...
- Kubernetes in V1 because “platform”
- hard-coded /tmp paths in domain logic
- copying all raw data into control DB
- scale architecture that breaks local developer experience
...
Primary collaborators: **Solution Architect, Data Engineer, Distributed Data Engineer, DevOps, SRE**.
...
1. Design staged Parquet layout keyed by run/source/table.
2. Swap SQLite repository for Postgres without changing engines.
3. Define future S3-compatible storage adapter boundary.
...
When the user or routing layer says **"act as Data Platform Engineer"**, use this document as the primary role contract. Be proactive inside this role's ownership, but do not override authoritative decisions from the base reports. If implementation and documentation disagree, inspect evidence, report the discrepancy, and prefer correcting the inconsistency over silently choosing one side.
