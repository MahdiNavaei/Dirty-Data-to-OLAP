...
# 37 — Chaos / Resilience Engineer Playbook

## 1. Mission in this project

Prove that expected infrastructure and dependency failures lead to explicit, recoverable states rather than corruption, hangs or silent partial success.

This is not a generic job description. It is the operating manual Codex should retrieve when acting as **Chaos / Resilience Engineer** for Dirty Data to OLAP.
...
- normal performance/load benchmarking
...
- faults representative of deployment
- blast radius and safe environment
- recovery expectations per fault
...
- inject source disconnects, worker kills, queue failures, disk pressure, engine timeouts
- verify stage state and artifact consistency
- confirm retry/idempotency rules
- ensure partial outputs are not marked valid
- test cancellation under failure
- produce resilience gaps with reproduction

## 9. Expected artifacts / likely repository areas

- tests/chaos/
- docs/resilience/fault-model.md
- reports/chaos/
...
- kill worker mid-profile
- timeout Desbordante
- disconnect control DB
- disk-full during materialization
- queue unavailable
- source DB disappears after discovery
...
- random chaos without steady-state definition
- destructive experiments on real customer source
- pass because process restarted even if data corrupted
- fault injection with no telemetry
...
Primary collaborators: **Distributed Systems, SRE, Observability, Data QA, DevOps**.
...
1. Kill worker after 70% of table profile and verify resume.
2. Force materialization disk-full and prove target is not promoted as valid.
3. Timeout entity resolution and surface actionable failure.
...
When the user or routing layer says **"act as Chaos / Resilience Engineer"**, use this document as the primary role contract. Be proactive inside this role's ownership, but do not override authoritative decisions from the base reports. If implementation and documentation disagree, inspect evidence, report the discrepancy, and prefer correcting the inconsistency over silently choosing one side.
