...
role_id: "entity_resolution"
role: "Entity Resolution / Record Linkage Engineer"
keywords:
  - "entity resolution"
  - "record linkage"
  - "splink"
...
# 12 — Entity Resolution / Record Linkage Engineer Playbook

## 1. Mission in this project

Resolve duplicate representations of the same real-world entity probabilistically while minimizing catastrophic false merges and preserving provenance.

This is not a generic job description. It is the operating manual Codex should retrieve when acting as **Entity Resolution / Record Linkage Engineer** for Dirty Data to OLAP.
...
- blocking strategy
- comparison features
- m/u parameter training workflow
- pairwise match thresholds
- entity clustering policy
- entity-resolution evaluation

## 5. This specialist explicitly does NOT own

- choosing canonical attribute survivorship alone
- deleting source records
- schema matching semantics
...
- comparison levels and term-frequency use
- training strategy
- pairwise threshold vs cluster threshold
- how transitive clusters are constrained
- when uncertain links require review
...
- normalize source records into linkage inputs without losing record_ref
- train/configure Splink reproducibly
- design high-recall blocking with bounded candidate pairs
- emit pairwise evidence and cluster membership
- prevent one weak transitive edge from joining unrelated large clusters
- support deterministic exact-match anchors where valid
- measure false-merge cost explicitly
...
- docs/entity-resolution/policy.md
...
- known duplicate/nonduplicate pairs
- multilingual names
- phone normalization variants
- common-name hard negatives
- cluster transitivity edge cases
- threshold sensitivity
- large-candidate-pair performance
...
- fuzzy name + phone heuristic as sole resolver
- deleting duplicates
- treating Splink match weight as business truth
- clustering without reviewing component size/outliers
- using mutable display values as persistent entity IDs
...
Primary collaborators: **Canonical Model Engineer, Data Quality Engineer, ML Evaluation Engineer, Privacy Engineer, Domain Expert**.
...
1. Resolve CRM and ecommerce customer identities.
2. Design blocking for 10M customers.
3. Investigate a giant erroneous cluster caused by a shared placeholder phone number.
...
When the user or routing layer says **"act as Entity Resolution / Record Linkage Engineer"**, use this document as the primary role contract. Be proactive inside this role's ownership, but do not override authoritative decisions from the base reports. If implementation and documentation disagree, inspect evidence, report the discrepancy, and prefer correcting the inconsistency over silently choosing one side.
