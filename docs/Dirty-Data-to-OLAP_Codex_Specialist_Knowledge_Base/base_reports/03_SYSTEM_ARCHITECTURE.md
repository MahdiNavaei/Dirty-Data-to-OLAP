...
Execution-plan preparation is phased because later planning truth is produced by the run itself. A fresh run persists a `BOOTSTRAP` plan for `SOURCE_DISCOVERY` only. After that owning stage succeeds, the durable coordinator resolves source-dependent conditional stages into a `SOURCE_RESOLVED` plan; after `CANONICAL_HYPOTHESES` succeeds, it resolves the final ER branch into `COMPLETE`. The same plan identity is compare-and-swapped through these revisions. Only artifacts attached to the succeeded owning stage attempt are eligible planning inputs, so unresolved or tampered future truth cannot be treated as an exclusion or used to bypass the scheduler.
