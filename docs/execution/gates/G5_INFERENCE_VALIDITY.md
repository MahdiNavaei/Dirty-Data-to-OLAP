...
Evidence: [Step18 review](../STEP18_ML_EVALUATION_REVIEW.md), [aggregate baseline](../../../reports/evaluation/STEP18_INFERENCE_BASELINE.md), [machine-readable summary](../../../reports/evaluation/STEP18_INFERENCE_BASELINE.json), and `workspace/runs/step18-inference-baseline-v4/evaluation/`. The v4 TEST relationship result is `5/7`; Fusion is actual and evaluated; schema and ER are unavailable; automation remains disabled.

The offline continuation verifies both optional runtimes from the local wheelhouse. Valentine executed the full estate, but Cupid is incomplete because 9/11 groups require absent NLTK corpora; Coma is complete 11/11. Schema profiling, quality, and dependency producers are complete for all 22 source results, but producer evidence does not authorize incomplete matcher evidence. Splink executes against all 24 `LINK_AND_DEDUPE` records but fails closed at truthful EM training completeness, so no ER metrics are claimed. This is an empirical blocker, not a permission to substitute defaults or heuristics. No automation threshold was selected and Step19 was not started.
