...
- Execution Plan Authority starts a fresh run with a durable bootstrap projection containing only SOURCE_DISCOVERY, then compare-and-swaps the same plan identity into source-resolved and complete phases as succeeded owning stages publish verified SourceCatalog/SourceSnapshotResult and CanonicalModelHypothesis artifacts. It owns conditional-stage selection, policy/evidence/scope fingerprints and fail-closed unresolved planning; clients cannot submit a complete authoritative selection.
