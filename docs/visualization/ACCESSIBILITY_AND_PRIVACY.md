...
Every rendered graph node has exactly one `AccessibleGraphRow` with a label, description, and related visual IDs; rows form an exact one-to-one closure over rendered node IDs and related IDs cannot point outside the rendered graph. Legends explain shape, line style, state, evidence, reliability, scope, review, and conflict fields in text. This is a nonvisual semantic equivalent for a future renderer; it is not a browser accessibility audit.
