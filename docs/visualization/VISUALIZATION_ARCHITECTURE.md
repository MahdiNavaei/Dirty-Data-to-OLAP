...
`VisualizationGraphInput` is a deliberately narrow, already-normalized input. It contains labels, state, evidence state, reliability, observation scope, reviewability, provenance, and relationship provenance. `VisualizationService` validates closure and uniqueness, applies explicit filters, selects a bounded node set, and emits `VisualizationGraph`.
...
The same projection supports a source/schema graph and specialized views. Specialized builders add semantic guards for lineage, evidence, profile, quality, validation, and measures; they do not reimplement domain truth.
