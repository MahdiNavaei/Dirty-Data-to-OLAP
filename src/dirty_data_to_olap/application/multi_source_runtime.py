...
from dirty_data_to_olap.domain.contracts.analytical import (
    AnalyticalInputBinding, AnalyticalInputDataset, DimensionSpec, FactSpec,
    GrainSpec, MeasureSpec,
)
from dirty_data_to_olap.domain.contracts.canonical import (
    ...
    EntityResolutionRequirement, ReviewCheckpoint,
)
...
class MultiSourceStageHandlers(LocalProductStageHandlers):
    ...
    def _dependency_all(self, request):
        ...
        for source_id in sorted(catalogs):
            ...
            result = self.product_policy.dependency_result(result, catalog=catalog)
    ...
    def _hypothesis(self, request):
        ...
        required = tuple(sorted(key for key, value in requirements.items() if value is EntityResolutionRequirement.ER_REQUIRED))
        return StageExecutionResult(status=StageResultStatus.SUCCEEDED, output_artifact_refs=(ref.artifact_id,), metadata={"entity_resolution_required": ",".join(required) or "none", "relationship_count": str(len(canonical_relationships))})
    ...
    def _entity_resolution(self, request):
        ...
        if not hypothesis.entity_resolution_specs:
            return StageExecutionResult(status=StageResultStatus.BLOCKED, failure_code="ER_NOT_APPLICABLE", failure_classification=FailureClassification.BLOCKED_PREREQUISITE, failure_reason="the selected domain policy does not require entity resolution")
        spec = hypothesis.entity_resolution_specs[0]
    ...
    def _identity_proposal(self, request):
        ...
        catalogs = {key: value for key, (_ref, value) in self._catalogs(request).items()}
        snapshots = {key: value for key, (_ref, value) in self._snapshots(request).items()}
        er_pair = self._optional_typed_from_run(request.run_id, "EntityResolutionResult", EntityResolutionResult)
        er_ref, er_result = (None, None) if er_pair is None else er_pair
        domain_assertions = tuple(value for _ref, value in self._all(request, "DomainAssertion", DomainAssertion).values())
        memberships = self.product_policy.identity_memberships(
            hypothesis=hypothesis,
            snapshots=snapshots,
            catalogs=catalogs,
            policy_ref=self.product_policy.provenance,
            entity_resolution_result=er_result,
            entity_resolution_ref=None if er_ref is None else er_ref.artifact_id,
            domain_assertions=domain_assertions,
        )
        self.product_policy.validate_identity_memberships(memberships)
        er_results = {} if er_result is None else {"customer": er_result}
        lineage_refs = [request.run_id, hypothesis_ref.artifact_id]
        if er_ref is not None:
            lineage_refs.append(er_ref.artifact_id)
        proposal = self.multi_source.identity_proposals.build(hypothesis=hypothesis, memberships=memberships, er_results=er_results, source_schema_fingerprints=hypothesis.source_schema_fingerprints, policy_refs=(self.product_policy.provenance,), provenance_refs=lineage_refs)
        ...
        return StageExecutionResult(status=StageResultStatus.SUCCEEDED, output_artifact_refs=(ref.artifact_id,), metadata={"membership_count": str(len(memberships)), "entity_resolution": "REQUIRED" if er_result is not None else "NOT_REQUIRED_BY_POLICY"})
    ...
    def _canonical(self, request):
        ...
        for source_id, catalog in catalogs.items():
            ...
            for item in snapshots[source_id].record_references:
                metadata[item.record_ref] = {"source_id": source_id, "snapshot_id": snapshots[source_id].snapshot.snapshot_id, "table_id": table.table_id}
        er_results = {} if er_result is None else {"customer": er_result}
        lineage_refs = [hypothesis_ref.artifact_id, proposal_ref.artifact_id]
        if er_ref is not None:
            lineage_refs.append(er_ref.artifact_id)
        model = self.multi_source.finalization.finalize(hypothesis=hypothesis, identity_proposal=proposal, identity_review=review, er_results=er_results, source_record_metadata=metadata, lineage_refs=lineage_refs, record_accounting_refs=(self.product_policy.record_accounting_ref(request.run_id),))
    ...
    def _typed_value(value, logical_type):
        """Keep the shared typed-value compatibility contract domain-neutral."""
    ...
    def _build_dataset_and_request(self, request, canonical):
        ...
        return self.product_policy.build_multi_source_dataset_and_request(
            run_id=request.run_id,
            canonical=canonical,
            service=self.multi_source,
            catalogs={key: value for key, (_ref, value) in self._catalogs(request).items()},
            snapshots={key: value for key, (_ref, value) in self._snapshots(request).items()},
    ...
    def _analytical(self, request):
        ...
        return StageExecutionResult(status=StageResultStatus.SUCCEEDED, output_artifact_refs=tuple(item.artifact_id for item in refs), metadata={"plan_id": plan.plan_id, "dataset_id": dataset.dataset_id, "dimensions": str(len(dimensions)), "facts": str(len(facts)), "measures": str(len(measures))})
