...
def test_dependency_adapter_emits_measured_candidates_and_cleans_ephemeral_input():
    ...
    try:
        result = DesbordanteDependencyAdapter(project_root=project_root, engine=_Engine(), reader=_Reader()).discover(request, catalog, snapshot_result, artifact_root=project_root / "artifacts")
