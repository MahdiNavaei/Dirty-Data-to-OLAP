...
def check_stage_graph(data: dict[str, Any]) -> tuple[set[str], int]:
    ...
    require(set(by_id["COMPILATION"].get("output_artifact_types", [])) >= {"CompiledPlan", "GeneratedSQL", "TargetConfig"}, "compilation must publish the typed target configuration alongside execution outputs")
    require(set(by_id["MATERIALIZATION"].get("input_artifact_types", [])) >= {"CompiledPlan", "GeneratedSQL", "TargetConfig", "ReviewDecision"}, "materialization must consume exact reviewed compiled SQL and target configuration")
...
def check_state() -> None:
    ...
    elif execution.get("current_step") == 11:
        require(execution.get("last_completed_step") == 10, "post-Step 10 state must record completed Step 10")
        require(execution.get("last_completed_role") == "data_security_privacy_engineer", "post-Step 10 state must record the privacy engineer")
    ...
    elif execution.get("current_step") == 13:
        require(execution.get("last_completed_step") == 12, "post-Step 12 state must record completed Step 12")
        require(execution.get("last_completed_role") == "dependency_discovery_engineer", "post-Step 12 role must be dependency_discovery_engineer")
    ...
    elif execution.get("current_step") == 15:
        require(execution.get("last_completed_step") == 14, "post-Step 14 state must record completed Step 14")
        require(execution.get("last_completed_role") == "entity_resolution_engineer", "post-Step 14 role must be entity_resolution_engineer")
    ...
    elif execution.get("current_step") == 18:
        require(execution.get("last_completed_step") == 17, "post-Step 17 state must record completed Step 17")
        require(execution.get("last_completed_role") == "evidence_fusion_engineer", "post-Step 17 role must be evidence_fusion_engineer")
    ...
    elif execution.get("current_step") == 22:
        require(execution.get("last_completed_step") == 21, "post-Step 21 state must record completed Step 21")
        require(execution.get("last_completed_role") == "analytical_semantic_layer_engineer", "post-Step 21 role must be analytical_semantic_layer_engineer")
        require(execution.get("current_role") == "data_qa_engineer", "post-Step 21 state must hand off to Step22")
        require("Data QA" in str(execution.get("current_specialist")), "current specialist must be Step22")
        require("Data QA" in str(execution.get("next_step")), "next step must be Step22")
    elif execution.get("current_step") == 23:
        require(execution.get("last_completed_step") == 22, "post-Step 22 state must record completed Step 22")
        require(execution.get("last_completed_role") == "data_qa_engineer", "post-Step 22 role must be data_qa_engineer")
    ...
    elif execution.get("current_step") == 25:
        require(execution.get("last_completed_step") == 24, "post-Step 24 state must record completed Step 24")
        require(execution.get("last_completed_role") == "distributed_data_engineer", "post-Step 24 role must be distributed_data_engineer")
        require(execution.get("current_role") == "ux_product_designer", "post-Step 24 state must hand off to Step25")
        require("UX" in str(execution.get("current_specialist")), "current specialist must be Step25")
        require("UX" in str(execution.get("next_step")), "next step must be Step25")
    elif execution.get("current_step") == 26:
        require(execution.get("last_completed_step") == 25, "post-Step 25 state must record completed Step 25")
        require(execution.get("last_completed_role") == "ux_designer", "post-Step 25 role must be ux_designer")
        require(execution.get("current_role") == "data_visualization_engineer", "post-Step 25 state must hand off to Step26")
        require("Data Visualization" in str(execution.get("current_specialist")), "current specialist must be Step26")
        require("Data Visualization" in str(execution.get("next_step")), "next step must be Step26")
    elif execution.get("current_step") == 27:
        require(execution.get("last_completed_step") == 26, "post-Step 26 state must record completed Step 26")
        require(execution.get("last_completed_role") == "data_visualization_engineer", "post-Step 26 role must be data_visualization_engineer")
        require(execution.get("current_role") == "senior_backend_engineer", "post-Step 26 state must hand off to Step27")
        require("Senior Backend" in str(execution.get("current_specialist")), "current specialist must be Step27")
        require("Senior Backend" in str(execution.get("next_step")), "next step must be Step27")
    elif execution.get("current_step") == 28:
        require(execution.get("last_completed_step") == 27, "post-Step 27 state must record completed Step 27")
        require(execution.get("last_completed_role") == "senior_backend_engineer", "post-Step 27 role must be senior_backend_engineer")
        require(execution.get("current_role") == "distributed_job_processing_engineer", "post-Step 27 state must hand off to Step28")
        require("Distributed Job Processing" in str(execution.get("current_specialist")), "current specialist must be Step28")
        require("Distributed Job Processing" in str(execution.get("next_step")), "next step must be Step28")
    elif execution.get("current_step") == 29:
        require(execution.get("last_completed_step") == 28, "post-Step 28 state must record completed Step 28")
        require(execution.get("last_completed_role") == "distributed_job_processing_engineer", "post-Step 28 role must be distributed_job_processing_engineer")
    ...
    elif execution.get("current_step") == 31:
        require(step30_g8_closed(state), "Step30/G8 closure must be sequential and coherent")
        require(execution.get("last_completed_step") == 30, "post-Step 30 state must record completed Step 30")
        require(execution.get("last_completed_role") == "devops_engineer", "post-Step 30 role must be devops_engineer")
    ...
    elif execution.get("current_step") == 33:
        require(step32_compatibility_closed(state), "Step32/compatibility closure must be sequential and coherent")
        require(execution.get("last_completed_step") == 32, "post-Step 32 state must record completed Step 32")
        require(execution.get("last_completed_role") == "compatibility_test_engineer", "post-Step 32 role must be compatibility_test_engineer")
    ...
    elif execution.get("current_step") == 36:
        require(step35_sre_closed(state), "Step35/SRE closure must be sequential and coherent")
        require(execution.get("last_completed_step") == 35, "post-Step 35 state must record completed Step 35")
        require(execution.get("last_completed_role") == "sre", "post-Step 35 role must be sre")
        require(execution.get("current_role") == "chaos_resilience", "post-Step 35 state must hand off to Step36")
        require("Chaos / Resilience Engineer" in str(execution.get("current_specialist")), "current specialist must be Step36")
        require("Chaos / Resilience Engineer" in str(execution.get("next_step")), "next step must be Step36")
    elif execution.get("current_step") == 37:
        require(step36_resilience_closed(state), "Step36/resilience closure must be sequential and coherent")
        require(execution.get("last_completed_step") == 36, "post-Step 36 state must record completed Step 36")
        require(execution.get("last_completed_role") == "chaos_resilience", "post-Step 36 role must be chaos_resilience")
        require(execution.get("current_role") == "performance_engineer", "post-Step 36 state must hand off to Step37")
        require("Performance Engineer" in str(execution.get("current_specialist")), "current specialist must be Step37")
        require("Performance Engineer" in str(execution.get("next_step")), "next step must be Step37")
    elif execution.get("current_step") == 38:
        require(step37_performance_closed(state), "Step37/performance closure must be sequential and coherent")
        require(execution.get("last_completed_step") == 37, "post-Step 37 state must record completed Step 37")
        require(execution.get("last_completed_role") == "performance_engineer", "post-Step 37 role must be performance_engineer")
        require(execution.get("current_role") == "load_stress", "post-Step 37 state must hand off to Step38")
        require("Load / Stress Test Engineer" in str(execution.get("current_specialist")), "current specialist must be Step38")
        require("Load / Stress Test Engineer" in str(execution.get("next_step")), "next step must be Step38")
    elif execution.get("current_step") == 39:
        require(step38_load_stress_closed(state), "Step38/load-stress closure must be sequential and coherent")
        require(execution.get("last_completed_step") == 38, "post-Step 38 state must record completed Step 38")
        require(execution.get("last_completed_role") == "load_stress", "post-Step 38 role must be load_stress")
    ...
    elif execution.get("current_step") == 41:
        require(step40_g14_closed(state), "Step40/G14 closure must be sequential and coherent")
        require(execution.get("last_completed_step") == 40, "post-Step 40 state must record completed Step 40")
        require(execution.get("last_completed_role") == "developer_experience_engineer", "post-Step 40 role must be developer_experience_engineer")
        require(execution.get("current_role") == "technical_writer", "post-Step 40 state must hand off to Step41")
        require("Technical Writer" in str(execution.get("current_specialist")), "current specialist must be Step41")
        require("Technical Writer" in str(execution.get("next_step")), "next step must be Step41")
