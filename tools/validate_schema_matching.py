...
def main() -> int:
    ...
    checks = {
        ...
        "step14 implementation is present and later handoff is explicit": (ROOT / "src/dirty_data_to_olap/adapters/entity_resolution").exists() and (("current_step: 15" in state and "last_completed_step: 14" in state) or ("current_step: 16" in state and "last_completed_step: 15" in state) or ("current_step: 17" in state and "last_completed_step: 16" in state) or ("current_step: 18" in state and "last_completed_step: 17" in state) or ("current_step: 19" in state and "last_completed_step: 18" in state) or ("current_step: 20" in state and "last_completed_step: 19" in state)) and ("G4_BOUNDED_INTELLIGENCE: \"PENDING\"" in state or "G4_BOUNDED_INTELLIGENCE: \"PASS\"" in state),
