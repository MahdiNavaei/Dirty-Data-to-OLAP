...
def main() -> int:
    ...
    checks = {
        ...
        "step14 implementation is present and Step15 handoff is explicit": (ROOT / "src/dirty_data_to_olap/adapters/entity_resolution").exists() and (("current_step: 15" in state and "last_completed_step: 14" in state) or ("current_step: 16" in state and "last_completed_step: 15" in state)) and "G4_BOUNDED_INTELLIGENCE: \"PENDING\"" in state,
        ...
        "formal G4 remains pending and G4A is intermediate": 'G4_BOUNDED_INTELLIGENCE: "PENDING"' in state and 'G4A_INDEPENDENT_EVIDENCE_PRODUCERS: "PASS"' in state and 'G4A_INDEPENDENT_EVIDENCE_PRODUCERS: "PENDING"' not in state,
    }
