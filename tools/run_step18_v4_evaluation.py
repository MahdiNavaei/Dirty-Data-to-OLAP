        ...
        for edge in result.edges:
            ...
            if pair.issubset(test_refs):
                if edge.model_prediction_band is EntityMatchPredictionBand.STRONG_LINK_EVIDENCE:
                    ...
                    review += 1
        clusters = [set(cluster.record_refs).intersection(test_refs) for cluster in result.clusters if len(set(cluster.record_refs).intersection(test_refs)) > 1]
    metrics = entity_resolution_metrics(
        predicted,
        truth_pairs,
        {key: value for key, value in test_entities.items() if len(value) > 1},
        clusters,
        evaluated_universe=universe,
        evaluated_record_refs=test_refs,
    ).model_dump(mode="json") if result else None
    return {"split": "TEST", "status": "EVALUATED" if result else "INSUFFICIENT_EVIDENCE", "prediction_method": "ACTUAL_ENTITY_RESOLUTION_RESULT_EDGES_AND_COMPLETE_PROJECT_PARTITION", "test_record_refs": sorted(test_refs), "evaluated_record_universe_count": len(test_refs), "evaluated_pair_universe_count": len(universe), "truth_positive_pair_count": len(truth_pairs), "truth_negative_pair_count": len(universe - truth_pairs), "review_edge_count": review, "metrics": metrics, "hard_negative_slices": {item["case_id"]: {"split": item["split"], "record_refs": item["record_refs"], "expected_kind": item["expected_kind"]} for item in groups if item["task"] == "ENTITY_RESOLUTION" and item["expected_kind"] == "negative"}}
