...
def main() -> int:
    ...
    _check(checks, "Step24 remains synchronous at the Step28 boundary", not (ROOT / "src/dirty_data_to_olap/application/stage_orchestrator.py").exists() and not (ROOT / "src/dirty_data_to_olap/application/job_control.py").exists() and (ROOT / "src/dirty_data_to_olap/application/jobs.py").exists(), "Step24 has no scheduler rewrite; durable job control is owned by Step28")
