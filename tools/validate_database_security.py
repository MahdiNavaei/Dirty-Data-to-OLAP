...
def main() -> int:
    ...
    checks.append(("execution is at a later specialist handoff", (execution.get("last_completed_step"), execution.get("current_step"), execution.get("current_role")) in {(14, 15, "applied_ml_engineer"), (15, 16, "llm_semantic_ai_engineer"), (16, 17, "evidence_fusion_engineer"), (17, 18, "ml_evaluation_engineer"), (18, 19, "canonical_model_engineer"), (19, 20, "olap_engineer"), (20, 21, "analytical_semantic_layer_engineer")}))
    checks.append(("G3 state and canonical gate agree", g3 == "PASS" and "Status: `PASS`" in gate_text))
    checks.append(("README agrees with G3 and specialist handoff", ("Steps 01-14 complete" in readme_text or "Steps 01-15 complete" in readme_text or "Steps 01-16 complete" in readme_text or "Steps 01-17 complete" in readme_text or "Steps 01-18 complete" in readme_text or "Steps 01-19 complete" in readme_text or "Steps 01-20" in readme_text) and "G0/G1/G2/G3 PASS" in readme_text and "Step15" in readme_text and "formal G3" not in readme_text.lower()))
    checks.append(("Step14 entity-resolution implementation is present", execution.get("last_completed_step") in {13, 14, 15, 16, 17, 18, 19, 20} and (ROOT / "src" / "dirty_data_to_olap" / "adapters" / "entity_resolution" / "splink.py").exists()))
