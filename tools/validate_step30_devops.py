...
def validate_state(checks: list[dict[str, Any]]) -> dict[str, Any]:
    from tools.execution_state import _step38_incomplete_handoff, step29_g7_closed, step30_g8_closed, step30_handoff, step31_external_ci_blocked, step31_qa_closed, step41_g15_closed
    ...
    values = {
        ...
        "step41_g15_closed": step41_g15_closed(state),
    ...
    step38_incomplete = values["step29_g7_closed"] and _step38_incomplete_handoff(state)
    blocked_external = values["step31_external_ci_blocked"]
    terminal_step41 = values["step41_g15_closed"]
    if not (terminal_step41 or pre_closure or post_closure or post_step31 or step38_incomplete or blocked_external):
    ...
    phase = "terminal-Step41" if terminal_step41 else "pre-G8" if pre_closure else "blocked-external-final-ci" if blocked_external else "post-Step31" if post_step31 else "step38-incomplete" if step38_incomplete else "post-G8"
